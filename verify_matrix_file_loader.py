...
# Carga dinámica directa de archivos .py en el directorio routers
for file in os.listdir(routers_dir):
    if file.endswith(".py") and not file.startswith("__"):
        mod_name = file[:-3]
        file_path = os.path.join(routers_dir, file)
        
        spec = importlib.util.spec_from_file_location(f"app.routers.{mod_name}", file_path)
