...
@Component({
  ...
  styles: [`
    .login-page {
      ...
      background: var(--bg-app); padding: 16px; box-sizing: border-box; position: relative;
    }
    .theme-toggle-btn { position: absolute; top: 20px; right: 20px; color: var(--text-main); }
    .login-card {
      width: 100%; max-width: 420px; padding: 36px 28px 20px 28px; border-radius: 16px;
      background: var(--bg-card); box-shadow: 0 10px 30px rgba(0, 0, 0, 0.15);
    ...
    .btn-submit {
      height: 48px; font-size: 1rem; font-weight: 700; border-radius: 8px;
...
export class LoginComponent {
  ...
  onSubmit(): void {
    ...
    setTimeout(() => {
      ...
      this.snackBar.open('¡Bienvenido a Marianix!', 'Aceptar', { duration: 2500 });
