...
import { DrogaFormDialogComponent, MonodrogaFormDialogComponent, PotenciaFormDialogComponent, ViaAdministracionFormDialogComponent, AccionTerapeurticaFormDialogComponent, MedicamentoFormDialogComponent } from '../dialogs/medicamento-form-dialog.component';
  ...
  template: `
    <div class="page-container notranslate" translate="no">
      ...
      <div class="card-table">
        ...
        <table mat-table [dataSource]="dataSource" matSort class="full-width-table">
          <ng-container matColumnDef="codigo"><th mat-header-cell *matHeaderCellDef mat-sort-header>Código</th><td mat-cell *matCellDef="let el" class="font-mono text-blue font-bold">{{el.codigo}}</td></ng-container>
          <ng-container matColumnDef="codigoSSS"><th mat-header-cell *matHeaderCellDef mat-sort-header>Código SSS</th><td mat-cell *matCellDef="let el" class="font-mono">{{el.codigoSSS || 'N/A'}}</td></ng-container>
          <ng-container matColumnDef="descripcion"><th mat-header-cell *matHeaderCellDef mat-sort-header>Descripción</th><td mat-cell *matCellDef="let el"><strong>{{el.descripcion}}</strong></td></ng-container>
          <ng-container matColumnDef="activo"><th mat-header-cell *matHeaderCellDef mat-sort-header>Estado</th><td mat-cell *matCellDef="let el"><span class="badge" [ngClass]="el.activo ? 'badge-active' : 'badge-inactive'">{{el.activo ? 'ACTIVO' : 'INACTIVO'}}</span></td></ng-container>
          <ng-container matColumnDef="acciones"><th mat-header-cell *matHeaderCellDef class="text-center">Acciones</th><td mat-cell *matCellDef="let el" class="text-center"><button mat-icon-button color="primary" (click)="openForm(el)"><mat-icon>edit</mat-icon></button><button mat-icon-button color="warn" (click)="onDelete(el)"><mat-icon>delete</mat-icon></button></td></ng-container>
          <tr mat-header-row *matHeaderRowDef="cols"></tr><tr mat-row *matRowDef="let row; columns: cols;"></tr>
...
@Component({
  ...
  template: `
    <div class="page-container notranslate" translate="no">
      <div class="header-actions">
        <div><h1>Padrón de Potencias</h1><p class="subtitle">Gestión de unidades de dosificación (MG, MCG, ML, UI, etc.)</p></div>
        <button mat-flat-button color="primary" (click)="openForm()"><mat-icon>add</mat-icon> Nueva Potencia</button>
      </div>
      <div class="card-table">
        <mat-form-field appearance="outline" class="search-field">
          <mat-label>Buscar potencia...</mat-label>
          <input matInput [(ngModel)]="searchTerm" (keyup)="applyFilter()" placeholder="Código, descripción o abreviatura...">
          <mat-icon matSuffix>search</mat-icon>
        </mat-form-field>
        <table mat-table [dataSource]="dataSource" matSort class="full-width-table">
          <ng-container matColumnDef="codigo"><th mat-header-cell *matHeaderCellDef mat-sort-header>Código</th><td mat-cell *matCellDef="let el" class="font-mono text-blue font-bold">{{el.codigo}}</td></ng-container>
          <ng-container matColumnDef="descripcion"><th mat-header-cell *matHeaderCellDef mat-sort-header>Descripción</th><td mat-cell *matCellDef="let el"><strong>{{el.descripcion}}</strong></td></ng-container>
          <ng-container matColumnDef="abreviatura"><th mat-header-cell *matHeaderCellDef mat-sort-header>Abreviatura</th><td mat-cell *matCellDef="let el" class="font-mono text-purple">{{el.abreviatura}}</td></ng-container>
          <ng-container matColumnDef="activo"><th mat-header-cell *matHeaderCellDef mat-sort-header>Estado</th><td mat-cell *matCellDef="let el"><span class="badge" [ngClass]="el.activo ? 'badge-active' : 'badge-inactive'">{{el.activo ? 'ACTIVO' : 'INACTIVO'}}</span></td></ng-container>
          <ng-container matColumnDef="acciones"><th mat-header-cell *matHeaderCellDef class="text-center">Acciones</th><td mat-cell *matCellDef="let el" class="text-center"><button mat-icon-button color="primary" (click)="openForm(el)"><mat-icon>edit</mat-icon></button><button mat-icon-button color="warn" (click)="onDelete(el)"><mat-icon>delete</mat-icon></button></td></ng-container>
          <tr mat-header-row *matHeaderRowDef="cols"></tr><tr mat-row *matRowDef="let row; columns: cols;"></tr>
        </table>
...
export class PotenciasPageComponent implements OnInit {
  ...
  applyFilter() { this.dataSource.filter = this.searchTerm.trim().toLowerCase(); }
  openForm(item?: any) {
    this.dialog.open(PotenciaFormDialogComponent, { width: '440px', data: item }).afterClosed().subscribe(res => {
      if (res) { this.snack.open('Potencia guardada con éxito', 'Aceptar', { duration: 2500 }); this.load(); }
    });
  }
  onDelete(item: any) {
...
// NUEVO COMPONENTE: ACCIONES TERAPÉUTICAS PAGE (FASE 6)
@Component({
  selector: 'app-acciones-terapeuticas-page',
  standalone: true,
  imports: [CommonModule, FormsModule, MatTableModule, MatPaginatorModule, MatSortModule, MatButtonModule, MatIconModule, MatInputModule, MatFormFieldModule, MatDialogModule, MatSnackBarModule],
  template: `
    <div class="page-container notranslate" translate="no">
      <div class="header-actions">
        <div><h1>Padrón de Acciones Terapéuticas</h1><p class="subtitle">Gestión de grupos farmacológicos y efectos terapéuticos</p></div>
        <button mat-flat-button color="primary" (click)="openForm()"><mat-icon>add</mat-icon> Nueva Acción</button>
      </div>
      <div class="card-table">
        <mat-form-field appearance="outline" class="search-field">
          <mat-label>Buscar acción terapéutica...</mat-label>
          <input matInput [(ngModel)]="searchTerm" (keyup)="applyFilter()" placeholder="Código o descripción...">
          <mat-icon matSuffix>search</mat-icon>
        </mat-form-field>
        <table mat-table [dataSource]="dataSource" matSort class="full-width-table">
...
export class AccionesTerapeurticasPageComponent implements OnInit {
  ...
  applyFilter() { this.dataSource.filter = this.searchTerm.trim().toLowerCase(); }
  openForm(item?: any) {
    this.dialog.open(AccionTerapeurticaFormDialogComponent, { width: '440px', data: item }).afterClosed().subscribe(res => {
      if (res) { this.snack.open('Acción terapéutica guardada con éxito', 'Aceptar', { duration: 2500 }); this.load(); }
    });
  }
  onDelete(item: any) {
    if (confirm(`¿Eliminar acción terapéutica '${item.descripcion}'?`)) {
      this.service.delete(item.id).subscribe(() => { this.snack.open('Acción eliminada correctamente', 'Aceptar', { duration: 2500 }); this.load(); }, (err: any) => alert(err.message));
    }
  }
}
